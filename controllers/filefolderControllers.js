...
const getFileFolders = async (req, res) => {
    ...
    try {
        ...
        // also get the shared folders, where the sharedWith array contains the email of the current user
        const user = await UserModel.findById(_id);
        const sharedFolders = await FileFolderModel.find({ sharedWith: { $in: [user.email] } });
        // console.log("Shared folders:", sharedFolders);

        fileFolders.push(...sharedFolders);
...
const shareFileFolder = async (req, res) => {
    ...
    // append the new email ids
    fileFolder.sharedWith = [...fileFolder.sharedWith, ...data];
    await fileFolder.save();
